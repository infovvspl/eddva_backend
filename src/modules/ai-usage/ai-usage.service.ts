...
export class AiUsageService implements OnModuleInit {
  ...
  // Rough, configurable cost rates (USD). Token-based for LLMs; flat per-request
  // for audio/char features where token counts aren't returned. Approximate.
  private static readonly TOKEN_RATE_PER_1K: Record<string, number> = {
    groq: 0.0006, groq_gemini: 0.0008, gemini: 0.001, groq_vision: 0.0008, whisper_llm: 0.0006,
  };
  ...
  private estimateCost(provider: string | null | undefined, totalTokens: number | null | undefined): number {
    const p = provider || '';
    if (totalTokens && AiUsageService.TOKEN_RATE_PER_1K[p]) {
      return +((totalTokens / 1000) * AiUsageService.TOKEN_RATE_PER_1K[p]).toFixed(6);
    }
    if (AiUsageService.REQUEST_RATE[p]) return AiUsageService.REQUEST_RATE[p];
    // Fallback: token-based at groq rate, else a tiny per-request default.
    if (totalTokens) return +((totalTokens / 1000) * 0.0006).toFixed(6);
    return 0.0005;
  }
  ...
  async record(ev: AiUsageEvent): Promise<void> {
    try {
      ...
      if ((ev.estCost === undefined || ev.estCost === null) && ev.success) {
        ev.estCost = this.estimateCost(ev.provider, ev.totalTokens);
      }
      await this.ds.query(
  ...
  async getRawLogs(opts: {
    ...
    if (opts.instituteId) {
      conditions.push(`institute_id = $${params.push(opts.instituteId)}`);
    }
    if (opts.vertical && opts.vertical !== 'all') {
      conditions.push(`vertical = $${params.push(opts.vertical)}`);
    }
    if (opts.feature) {
      conditions.push(`feature = $${params.push(opts.feature)}`);
    }
    if (opts.from) {
      conditions.push(`created_at >= $${params.push(opts.from + ' 00:00:00')}`);
    } else {
      const start = this.monthStart();
      conditions.push(`created_at >= $${params.push(start + ' 00:00:00')}`);
    }
    if (opts.to) {
      conditions.push(`created_at <= $${params.push(opts.to + ' 23:59:59')}`);
    }
    ...
    const countQuery = `SELECT COUNT(*)::int AS count FROM ai_usage_events${where}`;
    const dataQuery = `
      SELECT id, institute_id, vertical, feature, provider, model, success,
             status_code, latency_ms, prompt_tokens, completion_tokens, total_tokens, est_cost,
    ...
    const [countRes, dataRes] = await Promise.all([
      this.ds.query(countQuery, params.slice(0, params.length - 2)),
    ...
    return {
      ...
      data: dataRes.map(row => ({
        ...
        est_cost: parseFloat(Number(row.est_cost || 0).toFixed(6)),
  ...
  async getBillingReport(opts: {
    ...
    if (opts.from) {
      conditions.push(`day >= $${params.push(opts.from)}`);
    } else {
      const start = this.monthStart();
      conditions.push(`day >= $${params.push(start)}`);
    }
    if (opts.to) {
      conditions.push(`day <= $${params.push(opts.to)}`);
    }
    ...
    const query = `
      ...
      FROM ai_usage_daily
      ${where}
      GROUP BY month, institute_id, vertical, feature
      ORDER BY month DESC, cost DESC
    `;

    const rows = await this.ds.query(query, params);
    const enriched = await this.resolveInstituteNames(rows);
