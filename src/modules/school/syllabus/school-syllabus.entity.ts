...
export class SchoolSyllabusPlan extends SchoolBase {
  ...
  @Column({ default: 'NORMAL' }) priority: string; // LOW, NORMAL, HIGH, URGENT
  @Column({ nullable: true }) term: string; // Term 1, Term 2, Month
  @Column({ default: 'PLANNED' }) status: string; // PLANNED, IN_PROGRESS, COMPLETED, DELAYED
}
...
export class SchoolLessonPlan extends SchoolBase {
  ...
  @Column({ default: 'DRAFT' }) status: string; // DRAFT, SUBMITTED, SCHEDULED, IN_PROGRESS, COMPLETED
}
...
export class SchoolLessonCompletion extends SchoolBase {
  ...
  @Column({ name: 'completion_type', default: 'FULLY' }) completionType: string; // FULLY, PARTIALLY, NOT_COMPLETED
