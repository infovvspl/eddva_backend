...
export class SchoolAssignmentService {
  ...
  async list(user: any, query: any) {
    ...
    let filter = 'a.tenant_id=$1';

    if (user.role === 'STUDENT') {
      const profile: any[] = await this.ds.query(
        `SELECT sec.class_id FROM students s
         LEFT JOIN sections sec ON s.section_id = sec.id
         WHERE s.user_id = $1`,
        [user.id],
      );
      const classId = profile[0]?.class_id;
  ...
  async create(user: any, body: any, file?: Express.Multer.File) {
    ...
    const tenantId = user.instituteId || user.tenantId;
