...
export class BattleService {
  ...
  private async buildAiBattleQuestions(
    ...
    try {
      ...
      // Each source degrades independently: a slow or failing notes call never
      // takes down topic questions (see `settle` below).
      const seed = Math.floor(Math.random() * 1000000);
      // P0-4.5 (G2): these two calls are BOTH background-pool and share one tenant,
      // and background per-tenant capacity is 1. Run in parallel, the second call
      // could not be admitted until the first released its slot — it sat in the
      // admission wait for up to 60s and then took a 429, which allSettled silently
      // swallowed. The result was a much slower request that quietly lost one of its
      // two question sources.
      //
      // Serialised, each call takes the slot cleanly in turn. `settle` preserves the
      // exact { status, value | reason } shape allSettled produced, so the
      // degradation handling below is unchanged: either source can still fail on its
      // own without taking the other down.
      const settle = async <T>(run: () => Promise<T>): Promise<
        { status: 'fulfilled'; value: T } | { status: 'rejected'; reason: any }
      > => {
        try { return { status: 'fulfilled', value: await run() }; }
      ...
      const topicResult = await settle(() =>
