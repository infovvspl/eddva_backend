...
async function inspectDb() {
  try {
    ...
    const fks = await ds.query(`
      ...
      WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_name IN ('students', 'teachers', 'users');
    `);
    ...
    // Check indexes
    const idxs = await ds.query(`
      SELECT indexname, indexdef
      FROM pg_indexes
      WHERE tablename IN ('users', 'students', 'teachers', 'study_materials');
    `);
